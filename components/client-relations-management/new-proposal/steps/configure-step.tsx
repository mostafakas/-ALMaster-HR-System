...
import { useGetClientsQuery } from "@/lib/store/services/clientsApi";
...
export function ConfigureStep() {
  ...
  const isPresetDays = [7, 15, 21, 30, 45, 60].includes(draft.validityDays);
  const [isCustomDays, setIsCustomDays] = React.useState(
    !isPresetDays && Boolean(draft.validityDays),
  );
  const [customDaysValue, setCustomDaysValue] = React.useState(
    !isPresetDays && draft.validityDays ? String(draft.validityDays) : "14",
  );

  const selectedServices = React.useMemo(() => {
    if (draft.services && draft.services.length > 0) return draft.services;
    if (draft.service) return [draft.service];
    return [];
  }, [draft.services, draft.service]);

  const handleToggleService = (key: ProposalService) => {
    let updated: ProposalService[];
    if (selectedServices.includes(key)) {
      updated = selectedServices.filter((s) => s !== key);
    } else {
      updated = [...selectedServices, key];
    }
    updateDraft({
      services: updated,
      service: updated[0] || undefined,
    });
  };
  ...
  const apiClients: ProposalDraftClient[] = React.useMemo(() => {
    const rawList = (clientsData as any)?.data || clientsData;
  ...
  const allClients = React.useMemo(() => {
    const combined = [...extraClients, ...apiClients];
    for (const sample of SAMPLE_CLIENTS) {
      const sampleEmail = (sample.email || "").toLowerCase();
      if (!combined.some((c) => (c.email || "").toLowerCase() === sampleEmail)) {
        combined.push(sample);
      }
    }
    return combined;
  }, [extraClients, apiClients]);
  ...
  return (
    <div className="w-full bg-[#f8fafc] min-h-full p-[24px]">
      <div className="bg-white rounded-[20px] p-[32px] flex flex-col gap-[32px]">
        ...
        <WizardFieldSection
          label="Service"
          hint="Select one or multiple services to auto-import CMS content into the builder">
          <div className="grid grid-cols-3 gap-[12px]">
        ...
        <WizardFieldSection
          ...
          <div className="flex flex-col gap-[12px]">
            <div className="grid grid-cols-7 gap-[12px]">
              {VALIDITY_PILLS.map((p) => {
                const selected =
                  p.value === 0
                    ? isCustomDays
                    : !isCustomDays && draft.validityDays === p.value;
                return (
                  <SelectionCard
                    key={p.value}
                    selected={selected}
                    onClick={() => {
                      if (p.value === 0) {
                        ...
                        const num = parseInt(customDaysValue, 10);
                        updateDraft({
                          validityDays: isNaN(num) || num <= 0 ? 14 : num,
                        });
                      } else {
            ...
            {isCustomDays ? (
              <div className="flex items-center gap-[12px] bg-[#f8fafc] p-3 rounded-[12px] border border-[#0047ff]/20 w-fit">
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!isNaN(num) && num > 0) {
                        updateDraft({ validityDays: num });
                      }
                    }}
                    className="w-20 bg-white border border-[#e5e5e5] rounded-[8px] px-3 py-1.5 text-xs font-bold text-[#343434] outline-none focus:border-[#0047ff] focus:ring-1 focus:ring-[#0047ff]"
                    placeholder="e.g. 10"
                    autoFocus
                  />
                  <span className="text-xs font-bold text-[#343434]">Days</span>
                </div>
              </div>
            ) : null}
          </div>
