...
} from "@/lib/store/services/proposalsApi";
import { useCreateClientMutation } from "@/lib/store/services/clientsApi";
...
export function ReviewStep() {
  ...
  const [createProposalDraftApi] = useCreateProposalDraftMutation();
  const [updateProposalDraftApi] = useUpdateProposalDraftMutation();
  ...
  const saveProposal = React.useCallback(
    (sent: boolean) => {
      ...
      // Asynchronously sync with cloud backend API
      (async () => {
        try {
          ...
          const serviceEnumMap: Record<string, string> = {
            programming: "web_development",
            "content-writing": "digital_marketing",
            design: "ui_ux_design",
            "artificial-intelligence": "custom_software",
            marketing: "digital_marketing",
            finance: "custom_software",
          ...
          const langKey = draft.language || "english";
          const marketKey = draft.country || "saudi";
          const mappedService =
            (draft.service && serviceEnumMap[draft.service]) || "web_development";

          let finalClientId = draft.client?.id;
          const isSampleId =
            !finalClientId ||
            finalClientId.startsWith("client-") ||
            finalClientId.startsWith("c-") ||
            finalClientId === "demo-client";

          if (isSampleId && draft.client?.name) {
            try {
              const newClientRes: any = await createClientApi({
                name: draft.client.name,
                email: draft.client.email || `client_${Date.now()}@almaster.com`,
                country: draft.client.country || "Saudi Arabia",
                phone: draft.client.phone,
              }).unwrap();
              const payload = newClientRes?.data || newClientRes;
              if (payload?.id) finalClientId = payload.id;
            } catch {
              // ignore
            }
          }

          if (!finalClientId || isSampleId) {
            try {
              const defaultClientRes: any = await createClientApi({
                name: draft.client?.name || "AlMaster Enterprise Client",
                email: draft.client?.email || `client_${Date.now()}@almaster.com`,
                country: "Saudi Arabia",
              }).unwrap();
              const payload = defaultClientRes?.data || defaultClientRes;
              if (payload?.id) finalClientId = payload.id;
            } catch {
              // ignore
            }
          ...
          const created: any = await createProposalDraftApi({
            title: draft.headline || "Proposal",
            clientId: finalClientId,
            service: mappedService,
            format: "presentation",
            dimensions: draft.dimensions === "a4" ? "a4" : "powerpoint",
            language: langMap[langKey] || "en",
            market: marketMap[marketKey] || "saudi",
            description: draft.shortDescription || undefined,
          }).unwrap();

          const createdPayload = created?.data || created;
          if (createdPayload?.id) {
            await updateProposalDraftApi({
              ...
              draft: {
                ...
                aboutModule: draft.about as any,
                serviceModule: draft.serviceDetails as any,
                whyModule: draft.why as any,
                scopeModule: draft.scope as any,
                quotationModule: draft.quotation as any,
                supportModule: draft.support as any,
                whatWeNeedModule: draft.whatWeNeed as any,
              },
            }).unwrap();
          }
        } catch {
    ...
    [dispatch, draft, records, createProposalDraftApi, updateProposalDraftApi, createClientApi],
  );
