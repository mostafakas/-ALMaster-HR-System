...
import { PROPOSAL_SERVICE_META, getProposalServiceMeta } from "@/lib/types/proposal";
...
import { useGetProposalsQuery } from "@/lib/store/services/proposalsApi";
import type {
  ProposalFormat,
  ProposalLanguage,
  ProposalMarket,
...
export function ProposalsListPage({
  ...
  const { data: apiProposalsData } = useGetProposalsQuery();
  const {
    viewing,
    setViewing,
    handleView,
  ...
  const apiProposals: StoredProposal[] = React.useMemo(() => {
    if (!apiProposalsData?.data) return [];
    return apiProposalsData.data.map((p) => {
      const marketVal: ProposalMarket =
        p.market === "saudi" ? "saudi" : p.market === "gulf" ? "egypt" : "global";
      const langVal: ProposalLanguage =
        p.language === "ar" ? "arabic" : p.language === "en" ? "english" : "both";
      const formatVal: ProposalFormat =
        p.dimensions === "powerpoint" ? "pptx" : "docx";
      const serviceVal: ProposalService =
        (p.service as ProposalService) || "programming";
      ...
      return {
        ...
        service: serviceVal,
        client: {
          id: p.client?.id || p.clientId,
          name: p.client?.name || "Client",
          avatar: p.client?.avatarUrl || undefined,
        },
        market: marketVal,
        language: langVal,
        format: formatVal,
        ...
        expiresAt: p.validUntil
          ? p.validUntil.slice(0, 10)
          : new Date().toISOString().slice(0, 10),
        sentAt: p.sentAt || null,
        variant: langVal === "arabic" ? "arabic" : "english",
      };
    });
  }, [apiProposalsData]);

  const combinedProposals = React.useMemo(() => {
    const list = [...proposals];
    for (const ap of apiProposals) {
      if (!list.some((p) => p.id === ap.id || p.code === ap.code)) {
        list.push(ap);
      }
    }
    return list;
  ...
  const visible = React.useMemo(() => {
    return combinedProposals.filter((p) => {
      ...
      if (search) {
        ...
        const matches =
          ...
          getProposalServiceMeta(p.service).label.toLowerCase().includes(q);
        if (!matches) return false;
      }
      return true;
    });
  }, [combinedProposals, marketTab, search, filters]);
