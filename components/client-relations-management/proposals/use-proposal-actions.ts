...
import { useDeleteProposalMutation } from "@/lib/store/services/proposalsApi";
...
export function useProposalActions() {
  ...
  const [deleteProposalApi] = useDeleteProposalMutation();
  ...
  const handleArchive = React.useCallback(
    async (p: StoredProposal) => {
      ...
      setViewing(null);
      try {
        await deleteProposalApi(p.id).unwrap();
      } catch {
        // Safe fallback
      }
    },
    [dispatch, deleteProposalApi],
  );
