...
import { Clock, Home, FileText, Plus, LogOut } from "lucide-react";
...
/**
 * CRM top-nav. Aligned with Dashboard layout:
 *
 *  ┌──────────┐  ┌────────────────┐ ┌──┐  ┌───────────────┐ ┌──────┐
 *  │ ⏱ time   │  │ Home  Proposals│ │+│  │ System Switcher │ │Logout│
 *  └──────────┘  └────────────────┘ └──┘  └───────────────┘ └──────┘
 */
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <PillWrapper>
          <button
            ...
            className="bg-destructive/10 h-10 flex items-center gap-3 px-3 rounded-[8px] text-destructive hover:bg-destructive/20 transition-all outline-none cursor-pointer">
            <LogOut className="size-3.5" strokeWidth={2.2} />
            <span className="text-sm font-bold leading-none">Logout</span>
          </button>
        </PillWrapper>
      </div>
    </header>
