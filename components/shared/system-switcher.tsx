...
import { getAvailableSystems, type SystemId } from "@/lib/systems";
...
export function SystemSwitcher() {
  ...
  const availableSystems = React.useMemo(() => getAvailableSystems(), []);
  ...
  const hasMultipleSystems = availableSystems.length > 1;

  if (!hasMultipleSystems) {
    return (
      <div className="bg-muted p-1.5 rounded-xl flex items-center shrink-0">
        <div className="bg-secondary h-10 flex items-center gap-2 pl-1.5 pr-3 py-1.5 rounded-lg">
